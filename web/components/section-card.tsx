...
const accent: Record<Accent, { text: string; bg: string; border: string; rgb: string }> = {
  moss: {
    ...
    bg: 'bg-earth-moss/10',
    border: 'border-earth-moss/30',
    rgb: '90 125 74',
  },
  fern: {
    text: 'text-earth-fern',
    bg: 'bg-earth-fern/10',
    border: 'border-earth-fern/30',
    rgb: '123 168 150',
  },
  terracotta: {
...
export function RankRow({ rank, primary, secondary, metric, accent: a = 'moss' }: RankRowProps) {
  ...
  return (
    <li className="group flex items-center gap-4 py-2 px-2 -mx-2 rounded-lg hover:bg-bg-hover/50 transition-colors">
      <span
        className={cn(
          'font-display text-2xl tracking-tight num leading-none w-8 text-right opacity-50 group-hover:opacity-100 transition-opacity',
          c.text,
        )}
      >
      ...
      <div className="flex-1 flex items-baseline justify-between gap-3 min-w-0">
        <div className="min-w-0">
          <span
            className={cn(
              'text-sm font-medium text-text-primary truncate transition-colors group-hover:[color:rgb(var(--accent))]',
            )}
          >
            {primary}
          </span>
          {secondary && <span className="text-xs text-text-muted ml-2">{secondary}</span>}
        </div>
